...
public class NeedsProfile {
    ...
    public void consumeDrink(int amount) {
        this.thirst = Math.max(100, this.thirst + amount);
    }
    ...
    public void consumeFood(int amount, int protein, int carbs, int calories) {
        this.hunger = Math.max(100, this.hunger + amount);
        this.protein = Math.max(100, this.protein + protein);
        this.carbs = Math.max(100, this.carbs + carbs);
        this.calories = Math.max(100, this.calories + calories);
    }
    ...
    public void sleep(int ticks) {
        this.energy = Math.max(100, this.energy + (ticks));
    }
