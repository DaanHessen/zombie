...
def process():
    ...
    with gzip.open(CSV_FILE, 'wt', encoding='utf-8', newline='') as f:
        ...
        handler.apply_file(PBF_FILE, locations=True, idx='sparse_file_array,nodes_europe_roads.bin')
        
        print(f"\n  [OK] Extracted {handler.count:,} Roads -> {CSV_FILE}")
        
    if os.path.exists(PBF_FILE):
        os.remove(PBF_FILE)
    if os.path.exists('nodes_europe_roads.bin'):
        os.remove('nodes_europe_roads.bin')
    gc.collect()
