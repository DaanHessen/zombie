...
CSV_FILE = "north_america_roads.csv.gz"
...
def process():
    ...
    with gzip.open(CSV_FILE, 'wt', encoding='utf-8', newline='') as f:
        ...
        handler.apply_file(PBF_FILE, locations=True, idx='sparse_file_array,nodes_north_america_roads.bin')
        
        print(f"\n  [OK] Extracted {handler.count:,} Roads -> {CSV_FILE}")
        
    if os.path.exists(PBF_FILE):
        os.remove(PBF_FILE)
    if os.path.exists('nodes_north_america_roads.bin'):
        os.remove('nodes_north_america_roads.bin')
    gc.collect()
