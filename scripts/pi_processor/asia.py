...
def download_file(url, filename):
    print(f"Downloading https://download.geofabrik.de/asia-latest.osm.pbf...")
    max_retries = 150
    for attempt in range(max_retries):
        try:
            ...
            time.sleep(300)
    raise Exception(f"Failed to download {https://download.geofabrik.de/asia-latest.osm.pbf} after {{max_retries}} attempts.")
...
def process():
    ...
    with open(CSV_FILE, 'w', encoding='utf-8', newline='') as f:
        ...
        handler.apply_file(PBF_FILE, locations=True, idx='sparse_file_array,nodes_asia.bin')
    ...
    if os.path.exists(PBF_FILE):
        os.remove(PBF_FILE)
    if os.path.exists('nodes_asia.bin'):
        os.remove('nodes_asia.bin')
    gc.collect()
