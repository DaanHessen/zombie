...
PBF_FILE = "oceania-latest.osm.pbf"
CSV_FILE = "oceania.csv"
...
def download_file(url, filename):
    print(f"Downloading https://download.geofabrik.de/australia-oceania-latest.osm.pbf...")
    max_retries = 150
    for attempt in range(max_retries):
        try:
            ...
            time.sleep(300)
    raise Exception(f"Failed to download {https://download.geofabrik.de/australia-oceania-latest.osm.pbf} after {{max_retries}} attempts.")
...
def process():
    ...
    with open(CSV_FILE, 'w', encoding='utf-8', newline='') as f:
        ...
        handler.apply_file(PBF_FILE, locations=True, idx='sparse_file_array,nodes_oceania.bin')
    ...
    if os.path.exists(PBF_FILE):
        os.remove(PBF_FILE)
    if os.path.exists('nodes_oceania.bin'):
        os.remove('nodes_oceania.bin')
    gc.collect()
